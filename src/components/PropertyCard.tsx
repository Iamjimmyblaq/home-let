...
import { Property } from '@/data/seed';
  ...
  return (
    ...
    >
      <div className="relative aspect-[4/3] overflow-hidden bg-muted">
        ...
        {p.hasVirtualTour && (
          <Badge className="absolute bottom-3 left-3 bg-accent text-accent-foreground"><Eye className="h-3 w-3 mr-1" />360° Tour</Badge>
        )}
        <button
      ...
      <div className="p-4 space-y-3">
        ...
        <div className="flex items-center text-xs text-muted-foreground gap-1">
          <MapPin className="h-3 w-3" /> {p.location}, {p.state}
        </div>
        <div className="flex items-center gap-4 text-xs text-muted-foreground">
          ...
          <span className="flex items-center gap-1"><Maximize className="h-3.5 w-3.5" />{p.sqm}m²</span>
        </div>
